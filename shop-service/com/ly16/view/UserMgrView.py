...
def showUserMgrView(pros, users, orders, sign):
    ...
    if user_oper == 1:
        ...
        user_new = User(1, user_name, user_password, user_address, user_isadmin) #生成一个对象
        ...
        user = User(user_id, user_name, user_password, user_address, user_isadmin)
        upuse = update_user(user)
        if not upuse:
            print("修改用户信息失败！")
            showUserMgrView(pros, users, orders, sign)
        else:
        ...
        selt = select_user(user_id)
        if selt[0]:
            lists = list(selt[1])
            print("用户编号：%d" % lists[0])
            print("用户名称：%s" % lists[1])
            print("用户密码：%s" % lists[2])
            print("用户地址：%s" % lists[3])
            print("用户权限：%s" % lists[4])
        else:
            print("查询失败！")
    elif user_oper == 5:
        print("=" * 10 + "您已经退出系统" + "=" * 15)
